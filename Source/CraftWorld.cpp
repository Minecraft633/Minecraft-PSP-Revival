...
CraftWorld::CraftWorld()
{
	...
	playerZoneSize = Vector3(14 * mainOptions.horizontalViewDistance, 15 * mainOptions.verticalViewDistance, 14 * mainOptions.horizontalViewDistance);
...
void CraftWorld::InitBlockVector()
{
	...
	blockTypes.push_back(StoneBlock());
	...
	blockTypes.push_back(YellowGlassBlock());
	blockTypes.push_back(GreenGlassBlock());
	blockTypes.push_back(PinkGlassBlock());
	blockTypes.push_back(PurpleGlassBlock());
	blockTypes.push_back(RedGlassBlock());
	blockTypes.push_back(BlackGlassBlock());
	blockTypes.push_back(BlueGlassBlock());
	blockTypes.push_back(GlowstoneBlock());
	blockTypes.push_back(Soulsand());
	blockTypes.push_back(Netherrack());
	blockTypes.push_back(MagmaBlock());
	blockTypes.push_back(Redstoneoff());
	blockTypes.push_back(BlockofQuartz());
	blockTypes.push_back(ChiseledQuartzBlock());
	blockTypes.push_back(PillarQuartzBlock());
	blockTypes.push_back(white_concrete_powder());
	blockTypes.push_back(yellow_concrete_powder());
	blockTypes.push_back(pink_concrete_powder());
	blockTypes.push_back(gray_concrete_powder());
	blockTypes.push_back(purple_concrete_powder());
	blockTypes.push_back(blue_concrete_powder());
	blockTypes.push_back(green_concrete_powder());
	blockTypes.push_back(red_concrete_powder());
	blockTypes.push_back(black_concrete_powder());
	blockTypes.push_back(YellowGlassBlockPannel());
	blockTypes.push_back(GreenGlassBlockPannel());
	blockTypes.push_back(PinkGlassBlockPannel());
	blockTypes.push_back(PurpleGlassBlockPannel());
	blockTypes.push_back(RedGlassBlockPannel());
	blockTypes.push_back(BlackGlassBlockPannel());
	blockTypes.push_back(BlueGlassBlockPannel());
	blockTypes.push_back(Fire());
	blockTypes.push_back(GravitySandBlock());
}
...
void CraftWorld::InitItemVector()
{
	...
	itemTypes.push_back(YellowGlassBlockPannelItem());
	itemTypes.push_back(GreenGlassBlockPannelItem());
	itemTypes.push_back(PinkGlassBlockPannelItem());
	itemTypes.push_back(PurpleGlassBlockPannelItem());
	itemTypes.push_back(RedGlassBlockPannelItem());
	itemTypes.push_back(BlackGlassBlockPannelItem());
	itemTypes.push_back(BlueGlassBlockPannelItem());
...
void CraftWorld::initWorld(short worldSize, char worldHeight, short chunkSize)
{
...
void CraftWorld::initPutBlocksLight(const int xx, const int zz)
{
	int x, y, z;
	int level = 0;
...
void CraftWorld::initPutBlocksLight(const int xx, const int yy, const int zz)
{
	int x, y, z;
	int level = 0;
...
void CraftWorld::SaveCompressedWorld(std::string  filename)
{
	...
	//size
	gzwrite(saveFile, &WORLD_SIZE, sizeof(short));

	//size
	gzwrite(saveFile, &WORLD_HEIGHT, sizeof(char));

	//chunksize
	gzwrite(saveFile, &CHUNK_SIZE, sizeof(short));

	//cubes
	gzwrite(saveFile, m_Blocks, sizeof(unsigned short) * (WORLD_SIZE * WORLD_HEIGHT * WORLD_SIZE));

	//settings
	gzwrite(saveFile, m_BlockSettings, sizeof(unsigned short) * (WORLD_SIZE * WORLD_HEIGHT * WORLD_SIZE));
	...
	gzwrite(saveFile, &HG, sizeof(float));
	...
	gzwrite(saveFile, &worldSeed, sizeof(short));
...
void CraftWorld::LoadWorld(const char* filename)
{
	...
	//size
	fread(&WORLD_SIZE, sizeof(short), 1, pFile);

	//chunksize
	fread(&CHUNK_SIZE, sizeof(short), 1, pFile);

	//cubes
	m_Blocks = new block_t[WORLD_SIZE * WORLD_HEIGHT * WORLD_SIZE];
	fread(m_Blocks, sizeof(unsigned short), (WORLD_SIZE * WORLD_HEIGHT * WORLD_SIZE), pFile);
...
void CraftWorld::LoadCompressedWorld(std::string  filename)
{
	...
	//size
	gzread(saveFile, &WORLD_SIZE, sizeof(short));

	//size
	gzread(saveFile, &WORLD_HEIGHT, sizeof(char));

	//chunksize
	gzread(saveFile, &CHUNK_SIZE, sizeof(char));

	//cubes
	m_Blocks = new block_t[WORLD_SIZE * WORLD_HEIGHT * WORLD_SIZE];
	gzread(saveFile, m_Blocks, sizeof(unsigned short) * (WORLD_SIZE * WORLD_HEIGHT * WORLD_SIZE));

	m_BlockSettings = new block_t[WORLD_SIZE * WORLD_HEIGHT * WORLD_SIZE];
	gzread(saveFile, m_BlockSettings, sizeof(unsigned short) * (WORLD_SIZE * WORLD_HEIGHT * WORLD_SIZE));
	...
	gzread(saveFile, &Earned, sizeof(unsigned short));

	gzread(saveFile, &HP, sizeof(unsigned short));

	gzread(saveFile, &HG, sizeof(float));

	gzread(saveFile, &OS, sizeof(unsigned char));

	gzread(saveFile, &brightFactor, sizeof(float));

	gzread(saveFile, &starsFactor, sizeof(float));

	gzread(saveFile, &worldSeed, sizeof(short));
...
void CraftWorld::setTextureSize(short texture, short chunk, short animTexture, short animChunk, short itemTexture, short itemChunk)
{
	texutreSize = texture;
	textureChunk = chunk;
...
void CraftWorld::GetSkyColor()
{
	...
	//clear sky colors
	r2 = TextureManager::Instance()->getRed(0, 2, textureSkyColorId) << 24;
	...
	r2 = TextureManager::Instance()->getRed(0, 0, textureSkyColorId) << 24;
	...
	r2 = TextureManager::Instance()->getRed(0, 1, textureSkyColorId) << 24;
	...
	// rainy sky colors
	r2 = TextureManager::Instance()->getRed(0, 5, textureSkyColorId) << 24;
	...
	r2 = TextureManager::Instance()->getRed(0, 3, textureSkyColorId) << 24;
	...
	r2 = TextureManager::Instance()->getRed(0, 4, textureSkyColorId) << 24;
...
void CraftWorld::SetWorldTime(float time)
{
	if (time >= 22)
	{
	...
	if (worldDayTime <= 5 || worldDayTime >= 23)
	{
		westLight = 0.0f;
		lightColor = Vector3(0.3f, 0.3f, 0.3f);
		eastLight = 0.05f;
	}
	...
	else if (worldDayTime > 9 && worldDayTime <= 18)
	{
	...
	else if (worldDayTime == 23 || worldDayTime == 22)
	{
		lightColor = Vector3(0.3, 0.3, 0.3);
		westLight = 0.0f;
	}
...
block_t CraftWorld::_GetBlock(const int x, const int y, const int z) //Block at a given coordinate.
{
...
block_t CraftWorld::_GetBlockNoCheck(const int x, const int y, const int z) //Block at a given coordinate.
{
	return m_Blocks[x + y * WORLD_SIZE + z * WORLD_SIZE * WORLD_HEIGHT];
}


block_t& CraftWorld::GetBlock(const int x, const int y, const int z) //Block at a given coordinate.
{
	if (x >= 0 && x < WORLD_SIZE && z >= 0 && z < WORLD_SIZE && y >= 0 && y < WORLD_HEIGHT)
	{
		return m_Blocks[x + y * WORLD_SIZE + z * WORLD_SIZE * WORLD_HEIGHT];
...
block_t& CraftWorld::GetBlockNoCheck(const int x, const int y, const int z) //Block at a given coordinate.
{
	return m_Blocks[x + y * WORLD_SIZE + z * WORLD_SIZE * WORLD_HEIGHT];
}
...
void CraftWorld::GetSpecialBlockVerts(int i, BaseBlock* blockType)
{
	...
	if (blockType->blockModel == 5) // Not a common model.
	{
	...
	// Clear the cache or there will be errors.
...
void CraftWorld::GetItemVerts(int i, BaseItem* itemType)
{
	...
	int startPixelsY = 512 - ((itemType->textureRow + 1) * 16);
	...
	if (TextureManager::Instance()->Images[texture]->Width == 512)
	{
		for (int i = 15; i >= 0; i--)
		{
			for (int j = 15; j >= 0; j--)
...
void CraftWorld::buildmobVerts()
{
	...
	mPosition.push_back(new Vector3(x, y, z + pixel * 10)); //Pig Model
	mtextures.push_back(new Vector2(right, down));
	mPosition.push_back(new Vector3(x, y + pixel * 12, z + pixel * 8));
	mtextures.push_back(new Vector2(right, up));
	mPosition.push_back(new Vector3(x, y + pixel * 12, z));
	...
	mTriangle.push_back(new Vector3(iVertex, iVertex + 2, iVertex + 3));
	mTriangle.push_back(new Vector3(iVertex + 4, iVertex + 5, iVertex));
...
void CraftWorld::CreateFullMeshChunk(const int StartX, const int StartY, const int StartZ)
{
	...
	// If we are in player zone at start, build the map.
...
void CraftWorld::GetSpecialBlock(int x, int y, int z, int& iVertex, SimpleMeshChunk* MeshChunk, block_t Block, bool transparent)
{
	...
	if (blockType->blockModel == 5)
	{
		if (Block == WaterBlock::getID())
		{
			...
			if (upper_block == Lava::getID() || upper_block == WaterBlock::getID() || upper_block == IceBlock::getID())
			{
				...
				BuildWorldBlockPlane(blockType, x, -y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 0, 0), Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
	...
	if (blockType->blockModel == 4)
	{
		...
		BlockColorx2 = Vector3(BlockFinalLight(x, y, z), BlockFinalLight(x, y - 0.1f, z), BlockFinalLight(x, y, z));
	...
	if (blockType->blockModel == 11)
	{
		...
		if (!connectFront && !connectBack && !connectLeft && !connectRight)
		{
			BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(8, 1, 16), Vector3(8, 1, 16), Vector3(8, 1, 0), Vector3(8, 1, 0));
			BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(8, 1, 0), Vector3(8, 1, 0), Vector3(8, 1, 16), Vector3(8, 1, 16), true, false, 0);

			if (BlockSolid(x, y - 1, z) == false)
			{
				BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 1, 7), Vector3(16, 1, 7), Vector3(16, 1, 9), Vector3(0, 1, 9));
				BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 1, 0), Vector3(9, 1, 0), Vector3(9, 1, 16), Vector3(7, 1, 16));

				BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 1, 16), Vector3(9, 1, 16), Vector3(9, 1, 0), Vector3(7, 1, 0), 1, 0);
				BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 1, 9), Vector3(16, 1, 9), Vector3(16, 1, 7), Vector3(0, 1, 7), 1, 0);
			}
			if (BlockSolid(x, y + 1, z) == false)
			{
				BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 1, 16), Vector3(9, 1, 16), Vector3(9, 1, 0), Vector3(7, 1, 0), 1, 0);
			...
			BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 1, 8), Vector3(16, 1, 8), Vector3(16, 1, 8), Vector3(0, 1, 8), true, false, 5);
			BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 1, 8), Vector3(16, 1, 8), Vector3(16, 1, 8), Vector3(0, 1, 8));
		}
		else
		{
			if (connectLeft)
			{
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 1, 8), Vector3(8, 1, 8), Vector3(8, 1, 8), Vector3(0, 1, 8));
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 1, 8), Vector3(8, 1, 8), Vector3(8, 1, 8), Vector3(0, 1, 8));

				if (BlockSolid(x, y - 1, z) == false)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 1, 7), Vector3(8, 1, 7), Vector3(8, 1, 9), Vector3(0, 1, 9));
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 1, 9), Vector3(8, 1, 9), Vector3(8, 1, 7), Vector3(0, 1, 7), 1, 0);
				}
				if (BlockSolid(x, y + 1, z) == false)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 1, 9), Vector3(8, 1, 9), Vector3(8, 1, 7), Vector3(0, 1, 7), 1, 0);
			...
			if (connectRight)
			{
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(8, 1, 8), Vector3(16, 1, 8), Vector3(16, 1, 8), Vector3(8, 1, 8));
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(8, 1, 8), Vector3(16, 1, 8), Vector3(16, 1, 8), Vector3(8, 1, 8));

				if (BlockSolid(x, y - 1, z) == false)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(8, 1, 7), Vector3(16, 1, 7), Vector3(16, 1, 9), Vector3(8, 1, 9));
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(8, 1, 9), Vector3(16, 1, 9), Vector3(16, 1, 7), Vector3(8, 1, 7), 1, 0);
				}
				if (BlockSolid(x, y + 1, z) == false)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(8, 1, 9), Vector3(16, 1, 9), Vector3(16, 1, 7), Vector3(8, 1, 7), 1, 0);
			...
			if (connectBack)
			{
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(8, 1, 8), Vector3(8, 1, 8), Vector3(8, 1, 0), Vector3(8, 1, 0));
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(8, 1, 0), Vector3(8, 1, 0), Vector3(8, 1, 8), Vector3(8, 1, 8));

				if (BlockSolid(x, y - 1, z) == false)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 1, 0), Vector3(9, 1, 0), Vector3(9, 1, 8), Vector3(7, 1, 8));
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 1, 8), Vector3(9, 1, 8), Vector3(9, 1, 0), Vector3(7, 1, 0));
				}
				if (BlockSolid(x, y + 1, z) == false)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 1, 8), Vector3(9, 1, 8), Vector3(9, 1, 0), Vector3(7, 1, 0));
			...
			if (connectFront)
			{
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(8, 1, 16), Vector3(8, 1, 16), Vector3(8, 1, 8), Vector3(8, 1, 8));
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(8, 1, 8), Vector3(8, 1, 8), Vector3(8, 1, 16), Vector3(8, 1, 16));

				if (BlockSolid(x, y - 1, z) == false)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 1, 8), Vector3(9, 1, 8), Vector3(9, 1, 16), Vector3(7, 1, 16));
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 1, 16), Vector3(9, 1, 16), Vector3(9, 1, 8), Vector3(7, 1, 8));
				}
				if (BlockSolid(x, y + 1, z) == false)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 1, 16), Vector3(9, 1, 16), Vector3(9, 1, 8), Vector3(7, 1, 8));
			...
			if (connectsNum == 1)
			{
				if (connectLeft || connectRight)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(8, 1, 9), Vector3(8, 1, 9), Vector3(8, 1, 7), Vector3(8, 1, 7), true, false, 0);
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(8, 1, 7), Vector3(8, 1, 7), Vector3(8, 1, 9), Vector3(8, 1, 9), true, false, 0);
				}
				if (connectBack || connectFront)
				{
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(7, 1, 8), Vector3(9, 1, 8), Vector3(9, 1, 8), Vector3(7, 1, 8), true, false, 0);
					BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(7, 1, 8), Vector3(9, 1, 8), Vector3(9, 1, 8), Vector3(7, 1, 8), true, false, 0);
				}
			}
		}
	}

	if (blockType->blockModel == 12)
	{
		BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16), Vector3(0, 15, 16), Vector3(0, 15, 0), Vector3(0, 0, 0));
		BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0), Vector3(16, 15, 0), Vector3(16, 15, 16), Vector3(16, 0, 16));

		if (BlockTransparentOrSpecial(x, y - 0.1f, z) == true)
			BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0), Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));

		BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 15, 16), Vector3(16, 15, 16), Vector3(16, 15, 0), Vector3(0, 15, 0));
		BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 15, 0), Vector3(16, 15, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));

		if (BlockTransparentOrSpecial(x, y, z + 1) == true)
			BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16), Vector3(16, 0, 16), Vector3(16, 15, 16), Vector3(0, 15, 16));

		return;
	}
	/// Fire
	if (blockType->blockModel == 13)
	{
		BlockColorx1 = Vector3(BlockFinalLight(x, y, z), BlockFinalLight(x, y, z), BlockFinalLight(x, y, z));
		BlockColorx2 = BlockColorx1 * 0.9f;
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x, y, z + 1, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y + pixel * 22, z + 1, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y + pixel * 22, z, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y, z, left, down, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x, y + pixel * 22, z, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y + pixel * 22, z + 1, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y, z + 1, left, down, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x + 1, y, z + 1, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y + pixel * 22, z + 1, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y + pixel * 22, z, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y, z, left, down, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x + 1, y, z, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y + pixel * 22, z, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y + pixel * 22, z + 1, left, up, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x, y, z + 1, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 2, y + pixel * 22, z + 1, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 2, y + pixel * 22, z, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y, z, left, down, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x, y, z, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 2, y + pixel * 22, z, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 2, y + pixel * 22, z + 1, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y, z + 1, left, down, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x + pixel * 16, y, z, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 14, y + pixel * 22, z, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 14, y + pixel * 22, z + 1, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 16, y, z + 1, left, down, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x + pixel * 16, y, z + 1, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 14, y + pixel * 22, z + 1, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 14, y + pixel * 22, z, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + pixel * 16, y, z, left, down, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x, y + pixel * 22, z + pixel * 2, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y + pixel * 22, z + pixel * 2, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y, z, left, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y, z, right, down, light1.x, light1.y, light1.z);
		...
		if (canCreate == true)
		{
			...
			MeshChunk->vert(x, y, z, left, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y, z, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y + pixel * 18, z, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y + pixel * 18, z, left, up, light1.x, light1.y, light1.z);
		...
		if (canCreate == true)
		{
			...
			MeshChunk->vert(x, y, z + pixel * 16, left, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y, z + pixel * 16, right, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y + pixel * 18, z + pixel * 14, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y + pixel * 18, z + pixel * 14, left, up, light1.x, light1.y, light1.z);
		...
		if (canCreate)
		{
			...
			MeshChunk->vert(x, y + pixel * 18, z + pixel * 14, right, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y + pixel * 18, z + pixel * 14, left, up, light1.x, light1.y, light1.z);
			MeshChunk->vert(x + 1, y, z + pixel * 16, left, down, light1.x, light1.y, light1.z);
			MeshChunk->vert(x, y, z + pixel * 16, right, down, light1.x, light1.y, light1.z);
...
void CraftWorld::UpdateBlockPhysics(int id)
{
	...
	for (int z = StartZ; z < CHUNK_SIZE + StartZ; ++z)
	{
		for (int y = StartY; y < CHUNK_SIZE + StartY; ++y)
		{
			for (int x = StartX; x < CHUNK_SIZE + StartX; ++x)
			{
				...
				/*/Fancy Tree Shadow
				if (GetBlock(x, y, z) == LeavesBlock::getID())
				{
					if (GetBlock(x, y - 1, z) == 0)
					{
						GetBlock(x, y - 1, z) = Lava::getID();
						PhysicsUpdate = true;
					}
				}*/
				//falling Lava
				...
				if (GetBlock(x, y, z) == WaterBlock::getID())
				{
					if (GetBlock(x, y - 1, z) == 0)
					{
						GetBlock(x, y - 1, z) = WaterBlock::getID();
						PhysicsUpdate = true;
					}
				}
				//falling sand
				if (GetBlock(x, y, z) == GravitySandBlock::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = GravitySandBlock::getID();
						PhysicsUpdate = true;
					}
				}
				//falling gravel
				...
				//falling white_concrete_powder
				if (GetBlock(x, y, z) == white_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = white_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				/*/falling orange_concrete_powder
				if(GetBlock(x,y,z) == orange_concrete_powder::getID())
				{
					if(GetBlock(x,y-1,z) == 0)
					{
						GetBlock(x,y,z) = 0;
						GetBlock(x,y-1,z) = orange_concrete_powder::getID();
					}
				}
				//falling magenta_concrete_powder
				if(GetBlock(x,y,z) == magenta_concrete_powder::getID())
				{
					if(GetBlock(x,y-1,z) == 0)
					{
						GetBlock(x,y,z) = 0;
						GetBlock(x,y-1,z) = magenta_concrete_powder::getID();
					}
				}
				//falling light_blue_concrete_powder
				if(GetBlock(x,y,z) == light_blue_concrete_powder::getID())
				{
					if(GetBlock(x,y-1,z) == 0)
					{
						GetBlock(x,y,z) = 0;
						GetBlock(x,y-1,z) = light_blue_concrete_powder::getID();
					}
				}*/
				//falling yellow_concrete_powder
				if (GetBlock(x, y, z) == yellow_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = yellow_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				/*/falling lime_concrete_powder
				if(GetBlock(x,y,z) == lime_concrete_powder::getID())
				{
					if(GetBlock(x,y-1,z) == 0)
					{
						GetBlock(x,y,z) = 0;
						GetBlock(x,y-1,z) = lime_concrete_powder::getID();
					}
				}*/
				//falling pink_concrete_powder
				if (GetBlock(x, y, z) == pink_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = pink_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				//falling gray_concrete_powder
				if (GetBlock(x, y, z) == gray_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = gray_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				/*/falling light_gray_concrete_powder
				if(GetBlock(x,y,z) == light_gray_concrete_powder::getID())
				{
					if(GetBlock(x,y-1,z) == 0)
					{
						GetBlock(x,y,z) = 0;
						GetBlock(x,y-1,z) = light_gray_concrete_powder::getID();
					}
				}*/
				/*/falling cyan_concrete_powder
				if(GetBlock(x,y,z) == cyan_concrete_powder::getID())
				{
					if(GetBlock(x,y-1,z) == 0)
					{
						GetBlock(x,y,z) = 0;
						GetBlock(x,y-1,z) = cyan_concrete_powder::getID();
					}
				}*/
				//falling purple_concrete_powder
				if (GetBlock(x, y, z) == purple_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = purple_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				//falling blue_concrete_powder
				if (GetBlock(x, y, z) == blue_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = blue_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				/*/falling brown_concrete_powder
				if(GetBlock(x,y,z) == brown_concrete_powder::getID())
				{
					if(GetBlock(x,y-1,z) == 0)
					{
						GetBlock(x,y,z) = 0;
						GetBlock(x,y-1,z) = brown_concrete_powder::getID();
					}
				}*/
				//falling green_concrete_powder
				if (GetBlock(x, y, z) == green_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = green_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				//falling red_concrete_powder
				if (GetBlock(x, y, z) == red_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = red_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				//falling black_concrete_powder
				if (GetBlock(x, y, z) == black_concrete_powder::getID())
				{
					if (GetBlock(x, y - 1, z) == 0 || GetBlock(x, y - 1, z) == WaterBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x, y - 1, z) = black_concrete_powder::getID();
						PhysicsUpdate = true;
					}
				}
				//fire
				if (GetBlock(x, y, z) == Fire::getID())
				{
					if (GetBlock(x + 1, y, z) == WoodBlock::getID() || GetBlock(x + 1, y, z) == LeavesBlock::getID() || GetBlock(x + 1, y, z) == TNTBlock::getID() || GetBlock(x + 1, y, z) == BlackWoolBlock::getID() ||
						GetBlock(x + 1, y, z) == BlackWoolBlock::getID() || GetBlock(x + 1, y, z) == GrayWoolBlock::getID() || GetBlock(x + 1, y, z) == RedWoolBlock::getID() || GetBlock(x + 1, y, z) == PinkWoolBlock::getID() ||
						GetBlock(x + 1, y, z) == DarkGreenWoolBlock::getID() || GetBlock(x + 1, y, z) == GreenWoolBlock::getID() || GetBlock(x + 1, y, z) == BrownWoolBlock::getID() || GetBlock(x + 1, y, z) == YellowWoolBlock::getID() ||
						GetBlock(x + 1, y, z) == BlueWoolBlock::getID() || GetBlock(x + 1, y, z) == LightBlueWoolBlock::getID() || GetBlock(x + 1, y, z) == VioletWoolBlock::getID() || GetBlock(x + 1, y, z) == PastelWoolBlock::getID() ||
						GetBlock(x + 1, y, z) == DarkWoodBlock::getID() || GetBlock(x + 1, y, z) == WhiteWoodBlock::getID() || GetBlock(x + 1, y, z) == WoodenFence::getID() || GetBlock(x + 1, y, z) == OakHalfBlock::getID())
					{
						GetBlock(x, y, z) = 0;
						GetBlock(x + 1, y + 1, z) = Fire::getID();
						PhysicsUpdate = true;
						int random = rand() % 10;

						if (random == 1)
						{
							RemoveLigtSourceAtPosition(x, y, z, Fire::getID());
							UpdateLightAreaIn(Vector3(x, y, z));
							GetBlock(x, y, z) = 0;
						}
					}
		...
		for (int y = StartY; y < CHUNK_SIZE + StartY; ++y)
		{
			for (int x = StartX; x < CHUNK_SIZE + StartX; ++x)
			{
				...
				if (worldWeather->itIsSnowing && worldWeather->GetWeatherType() == 1 && y != CHUNK_SIZE + StartY - 1)
				{
					if (rand() % 20 < 3)
					{
						if (blockTypes[block].blockModel == 0) // Snow can only be placed on normal blocks.
						{
				...
				{
					if (BlockFinalLight(x, y, z) > 0.5f) // Mooshrooms can only grow in dark places.
						return;
				...
				{
					GetBlock(x + 1, y, z) = CobbleStone::getID(); // Make CobbleStone.
					continue;
				}
				if (block == Lava::getID() && GetBlock(x - 2, y, z) == WaterBlock::getID() && GetBlock(x - 1, y, z) == 0)
				{
			...
			for (int x = StartX; x < CHUNK_SIZE + StartX; ++x)
			{
				...
				if (x > 0 && x < WORLD_SIZE && y > 0 && y < WORLD_SIZE && z > 0 && z < WORLD_SIZE)
				{
					...
					if (block == Soil::getID())
					{
						...
						if (!worldWeather->itIsRaining) // If the weather's clear.
						{
							if (waterFoundNearBy)
							{
								if (rand() % 10 > 3)
					...
					if (block == Mooshroom1::getID())
					{
						if (block_light > 0.5f) // Mooshrooms can only grow in dark places.
							return;
					...
					if (block == 122 && (GetBlock(x, y - 1, z) == 1 || GetBlock(x, y - 1, z) == 2 || GetBlock(x, y - 1, z) == 29))
					{
						...
						if ((rand() % 22 < 2 && (GetBlock(x, y - 1, z) == 1 || GetBlock(x, y - 1, z) == 2)) || (rand() % 19 == 0 && GetBlock(x, y - 1, z) == 29))
						{
							...
							// Check if there's water or another tree near.
							bool flag = 1;
...
void CraftWorld::drawWhiteCube(int i)
{
	DrawPlaceTexture(false);
...
void CraftWorld::drawHandItems(int i, float light)
{
	...
	sceGuColor(GU_COLOR(light * 0.5, light * 0.5, light * 0.5, 1));
...
void CraftWorld::drawFull3DItem(int i, float light)
{
	...
	DrawSetCulling(false);
	sceGuColor(GU_COLOR(light * 0.9, light * 0.9, light * 0.9, 1));
	sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, itemTypes[i - 250].add2Points, 0, itemTypes[i - 250].add2Vertices);
...
void CraftWorld::drawBody(int leggingsId, float light)
{
...
void CraftWorld::UpdateWorldAnimation(float dt)
{
	if (mainOptions.worldBlockAnimation == true)
	{
		...
		if (animationTimer >= animationTimerEnd) // every 0.075 secs
		{
			...
			animationFireFrame += 1; // go to the next water frame
			...
			{
				animationFireFrame = 0; // zeroize frames
			}
			...
			for (int i = 0; i < waterFrameSize; i++)
			{
				for (int j = 0; j < waterFrameSize; j++)
				{
					...
					TextureManager::Instance()->setColour(waterFrameSize * 15 + i,
						waterFrameSize * 14 + j,
						TextureManager::Instance()->getRed(i, animationFireFrame * waterFrameSize + j, textureFireAnimationId),
						TextureManager::Instance()->getGreen(i, animationFireFrame * waterFrameSize + j, textureFireAnimationId),
						TextureManager::Instance()->getBlue(i, animationFireFrame * waterFrameSize + j, textureFireAnimationId),
						TextureManager::Instance()->getAlpha(i, animationFireFrame * waterFrameSize + j, textureFireAnimationId),
						textureTerrainId);
...
void CraftWorld::SpawnMobs(float dt)
{
	...
	if (spawnZombiesTimer > 15.0f)
	{
		...
		{
			if (rand() % 50 <= 26 + (24 * mZombies.empty()) && mZombies.size() < 4)
			{
				float spawnX = 0;
				float spawnY = 0;
				float spawnZ = 0;
		...
		{
			if (rand() % 50 <= 30 + (24 * mZombies.empty()) && mZombies.size() < 4)
			{
				float spawnX = 0;
				float spawnY = 0;
				float spawnZ = 0;
	...
	if (spawnCowsTimer < 0.0f)
	{
		...
		{
			if (rand() % 2 == 0 && mCows.size() + mSheeps.size() + mPIGs.size() < 12)
			{
				float spawnX = 0;
				float spawnY = 0;
				float spawnZ = 0;

				rand() % 2 == 0 ? spawnX = playerPos.x - 2 - rand() % 18 : spawnX = playerPos.x + 2 + rand() % 18;
				rand() % 2 == 0 ? spawnZ = playerPos.z - 2 - rand() % 18 : spawnZ = playerPos.z + 2 + rand() % 18;
				spawnY = groundHeightWater(spawnX, spawnZ);
		...
		spawnCowsTimer = 80 + (mPIGs.size() + mCows.size() + mSheeps.size()) * 10 + rand() % 60;
	}
	...
	if (spawnPIGsTimer < 0.0f)
	{
		...
		spawnPIGsTimer = 80 + (mPIGs.size() + mCows.size() + mSheeps.size()) * 10 + rand() % 60;
	}
	...
	if (spawnSheepTimer < 0.0f)
	{
		if (worldDayTime > 6 && worldDayTime < 21)
		{
			if (rand() % 2 == 0 && mCows.size() + mSheeps.size() + mPIGs.size() < 10)
			{
				float spawnX = 0;
				float spawnY = 0;
				float spawnZ = 0;

				rand() % 2 == 0 ? spawnX = playerPos.x - 4 - rand() % 18 : spawnX = playerPos.x + 4 + rand() % 18;
				rand() % 2 == 0 ? spawnZ = playerPos.z - 4 - rand() % 18 : spawnZ = playerPos.z + 4 + rand() % 18;
				spawnY = groundHeightWater(spawnX, spawnZ);
		...
		spawnSheepTimer = 100 + (mPIGs.size() + mCows.size() + mSheeps.size()) * 10 + rand() % 60;
	}
}
...
void CraftWorld::UpdateWorldTime(float dt)
{
	...
	worldHour += dt;
	if (worldHour >= 50.0f) // One game hour takes 50 seconds.
	{
		worldHour -= 50.0f;
		worldDayTime += 1.0f;
...
int CraftWorld::BlockSoundAtPos(Vector3 pos)
{
	...
	// Outside world collision.
	if (x <= 0 || y <= 0 || z <= 0 || x >= WORLD_SIZE - 1 || y >= WORLD_HEIGHT - 2 || z >= WORLD_SIZE - 1) return -1;
...
bool CraftWorld::SolidAtPoint(Vector3 pos)
{
	...
	// Don't collide with water or air.
	...
	if (blockTypes[posBlockID].blockModel == 1) // If it's a half block.
	{
		if (relative_y < 0.5f)
		{
			return true;
...
bool CraftWorld::SolidAtPointForPlayerOnGround(Vector3 pos)
{
	...
	// Outside world collision.
	if (x <= 0 || z <= 0 || y <= 0 || y >= WORLD_HEIGHT - 1 || x >= WORLD_SIZE - 1 || z >= WORLD_SIZE - 1) return true;
		...
		for (float t = 0; t < PI; t += longitude_increment)
		{
			...
			if (x > X)
			{
				for (int x2 = x; x2 >= X - (x2 - X); x2--)
				{
					...
					if (block == bedrock::getID() || block == Obsidian::getID() || block == WaterBlock::getID() || block == 0)
					{
						continue;
					}
	...
	for (float u = 0; u < PI * 2; u += latitude_increment)
	{
		for (float t = 0; t < PI; t += longitude_increment)
		{
			...
			if (x > X)
			{
				for (int x2 = x; x2 >= X - (x2 - X); x2--)
				{
					if (GetBlock(x2, y, z) == bedrock::getID() || GetBlock(x2, y, z) == WaterBlock::getID())
					{
						continue;
					}
...
void CraftWorld::BuildWorldBlockPlane(BaseBlock* blockType, int x, int y, int z, int& iVertex, SimpleMeshChunk* MeshChunk, block_t Block, bool transparent, char side, Vector3 point1, Vector3 point2, Vector3 point3, Vector3 point4)
{
	...
	if (side == 1)
	{
		...
		if (transparent == true)
		{
			if ((transparentBlock == true && Block1 != Block) || Block1 == 0 || (specialBlock > 0 && Block1 != 4 && Block1 != 112 && Block1 != 9 && Block1 != 37 && Block1 != 247 && Block1 != 38))
			{
				canCreate = true;
			}
			if (transparentBlock == false && specialBlock == 0)
