...
namespace Aurora
{
	namespace Graphics
	{
		...
		void ObjModel::LoadObj(const char* FileName)
		{
			...
			while (!feof(fp))
			{
				...
				case 'u'://use material
				{
					...
					fgets(strLine, 100, fp);// read line to the end
				}
				break;
		...
		void ObjModel::Optimize()
		{
			for (unsigned int i = 0; i < mMeshes.size(); i++)
			{
				...
				for (unsigned int f = 0; f < mesh->mFace.size(); f++)
				{
					...
					vert1.u = allUVMap[mesh->mUVFace[f].x].u;
					vert1.v = allUVMap[mesh->mUVFace[f].x].v;
					vert1.x = allVertex[mesh->mFace[f].x].x;
					vert1.y = allVertex[mesh->mFace[f].x].y;
					vert1.z = allVertex[mesh->mFace[f].x].z;
					memcpy(&mesh->meshVertices[j], &vert1, sizeof(TexturesPSPVertex));
					mesh->aabb.expandToInclude(allVertex[mesh->mFace[f].x]);
					j++;
					TexturesPSPVertex vert2;
					vert2.u = allUVMap[mesh->mUVFace[f].y].u;
					vert2.v = allUVMap[mesh->mUVFace[f].y].v;
					vert2.x = allVertex[mesh->mFace[f].y].x;
					vert2.y = allVertex[mesh->mFace[f].y].y;
					vert2.z = allVertex[mesh->mFace[f].y].z;
					memcpy(&mesh->meshVertices[j], &vert2, sizeof(TexturesPSPVertex));
					mesh->aabb.expandToInclude(allVertex[mesh->mFace[f].y]);
					j++;
					TexturesPSPVertex vert3;
					vert3.u = allUVMap[mesh->mUVFace[f].z].u;
					vert3.v = allUVMap[mesh->mUVFace[f].z].v;
					vert3.x = allVertex[mesh->mFace[f].z].x;
					vert3.y = allVertex[mesh->mFace[f].z].y;
					vert3.z = allVertex[mesh->mFace[f].z].z;
		...
		void ObjModel::LoadOptimized(const char* FileName)
		{
			...
			if (!binaryFile)
			{
				Utils::Logger::Instance()->LogMessage("Missing Model: \"%s\"!\n", FileName);
				return;
			}
