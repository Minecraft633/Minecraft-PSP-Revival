#include <Source/Aurora/Graphics/Image.h>
#include <Source/Aurora/System/FileManager.h>
		...
		bool Image::loadImageFromMemory(std::string newName,void *data,std::size_t size)
		{
			if (data && size)
			{
				...
				if (ptr && imgWidth && imgHeight)
				{
					...
					_width  = imgWidth;
					_height = imgHeight;
					_pixels = new unsigned char[_width * _height * 4];// Copy the loaded pixels to the pixel buffer
					memcpy(_pixels, ptr, _width * _height * 4);
		
					stbi_image_free(ptr);// Free the loaded pixels (they are now in our own pixel buffer)
					delete [] data;
