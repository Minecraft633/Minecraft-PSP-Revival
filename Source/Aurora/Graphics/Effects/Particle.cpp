...
namespace Aurora
{
	namespace Graphics
	{
		Particle::Particle(CraftWorld* world, std::string _name, Vector3 _position)
		{
			...
			if (name == "xp_orb")
			{
				texture = TextureHelper::Instance()->GetTexture(TextureHelper::xp_orbTexture);
			...
			if (name == "block crack")
			{
				texture = TextureHelper::Instance()->GetTexture(TextureHelper::Terrain3);

				SetLiveTime(600, 1200);
				SetScale(0.04f, 0.11f);
				SetFramesAmount(1);
				SetBrightness(1.0f);
				SetGravity(-6.8f);
		...
		void Particle::BuildVerticies()
		{
			...
			particleVertices[i].z = position.z + right.z - up.z;// * stepScale;
			particleVertices[i].u = frame * frameSize + frameSize;// * textureScale;
			particleVertices[i].v = 1.0f;// * textureScale;
		...
		{
			float texturePixel = (float)1.0f / 512.0f;
			float percent = (float)1.0f / 32.0f;
