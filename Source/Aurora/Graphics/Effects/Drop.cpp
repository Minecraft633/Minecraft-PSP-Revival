...
void Drop::Render(Frustum &camFrustum, Vector3 playerPos, float camAngle)
{
    ...
    if(toDestroy == false && getMe == false && active == true)
    {
        if(abs(position.x-playerPos.x) < 1.2f && abs(position.y-playerPos.y) < 1.8f && abs(position.z-playerPos.z) < 1.2f && existingTime >= 0.5f && myCrft->InventoryIsFull(id,amount,stackable) == false && myCrft->HP > 0.0f)
        {
            getMe = true;
            return;
        }
