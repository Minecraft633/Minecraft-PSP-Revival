...
namespace Aurora
{
	namespace Graphics
	{
		...
		void RenderManager::InitDebugFont()
		{
			...
			Font = intraFontLoad("Assets/font.pgf",INTRAFONT_CACHE_ASCII);

			intraFontSetStyle(Font,0.5f,WHITE,0,INTRAFONT_ALIGN_CENTER);
		}

		void RenderManager::SetFontStyle(float size, unsigned int color, unsigned int shadowColor, unsigned int options)
		{
			intraFontSetStyle(Font, size, color, 0, options);
		}
        ...
        void RenderManager::SetFont(int type)
		{
			...
			switch(type)
			{
			    case 0: Font = Font; fontVerticalShift = -4; break;
			}
		}
		...
		void RenderManager::DebugPrint(int x,int y, const char *message, ...)
		{
			...
			intraFontPrint(Font, x, y+fontVerticalShift, cbuffer);
